...
IMAGE_NVIDIA=$(echo "${BASE_IMAGE}" | grep -o "asus-nvidia" | return 0 || echo "${BASE_IMAGE}" | grep -o "surface-nvidia" | return 0) 
IMAGE_DEVICES=$(echo "${BASE_IMAGE}" | sed 's/asus-nvidia//' | grep -o "asus"  | return 0 || echo "${BASE_IMAGE}" | sed 's/surface-nvidia//' | grep -o "surface" | return 0)
